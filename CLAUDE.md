...
All three surfaces are styled from one stylesheet (ADR-0008, ADR-0009, ADR-0010 carry the measurements and the reasoning; the tokens carry their contrast ratios in comments beside them). The rules:
...
- `Callout` is a rule and its text, never a card and never a shadcn `Alert`. The tone lives in the rule alone; the prose stays muted and there is no icon, so nothing depends on colour alone.
...
**The Launcher is the same ledger; the Popover deliberately is not** (ADR-0009, ADR-0010). Neither window adds a token, keyframe, or line of CSS of its own.
