...
// becomes twelve. `src/lib/saveSlot.svelte.ts` carries the shared half of that
// reasoning, and `store.svelte.ts` is the other half of this window — the
// global config, which is also where the defaults an override inherits from
// and the model catalog are already loaded, so this store never fetches them.
//
